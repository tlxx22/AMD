...
本节保持规划权限。P2原24-run序列已结束，development adequacy=Not passed，工程implementation/review/closure的Passed/Completed不变（M4 §55）。用户要求优先评估独立run并发：**M4先做已可执行UrbanEV AMD-Concat的瓶颈/有限1路与2路预检；M5再确认实际选定模型的执行配置；M6依据批准清单调度，模型独占效率与任务组调度效率分开报告。** 这是执行规划，不表示M5/M6已开始或整体阶段重排获批。当前只允许只读资源/执行路径核对、仓库外脚本与文档准备；不运行新训练、并发负载或模型profiling。M4 §55给出唯一短测Proposed包：每进程原batch128/threads4/workers0与原seed/LR/精度，3次同一对任务串行/2路对照，另一个独占短trace，共拟496个Adam步骤；预算、时间/空间上限、数值容差及执行须另经批准。批量增大、workers/线程调整、4路、AMP/TF32/compile及MPS均不混入首轮。
