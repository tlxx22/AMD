...
| Extrapolated | P2 三臂约 14.49 h（历史规划值） | M4 §47.8以旧AMD control：ETTm1 40.220 min、UrbanEV 249.620 min，各乘三；不是P2实测或保证上界；ETTm1已完成，原后半UrbanEV12-run现获执行授权但尚未启动（M4 §54） |
...
本节只设计；现场资源只读快照见M4 §48.4，不能据此承诺未来GPU独占、CPU/RAM配额或可用存储。以下P0–P3是效率预检步骤编号；其中P2不授予PMCR P2结构或训练权限；P2生产工程现已review/closure，当前ETTm1执行及有限固定batch smoke授权以M4 §53为准，不据此启动本节的batch搜索或并发预检。后续须先审核代表任务、测量重复数、warm-up、数值容差、精确probe预算及命令，再执行获准预检。
...
阶段任务表本轮保持原文；其中M4行的§53执行状态仅保留为历史时点，当前执行授权与结果按§0.1及M4 §54读取。仅登记“UrbanEV完成后再处理M4/M5/M6精简安排”为后续事项，本轮不重排阶段、不冻结正式variant或解除组合guard。
...
当前：MS文档、A/B接口及P2生产实现review/Git closure均已完成；C production implementation complete、engineering gate=Passed，有限工程范围及skip见M4 §51–52；ETTm112-run完整性及C vs B/C vs A安全gate已获接受，未显示P2优于v1；原24-run合同中的UrbanEV后半12-run现获授权，本轮仅准备，文档审核/closure后由用户启动；UrbanEV尚未启动，P2整体adequacy待判，M4 In Progress（M4 §54）
