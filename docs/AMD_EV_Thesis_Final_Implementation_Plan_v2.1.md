...
### 9.6.4 A800 batch / 并发效率预检（短测完成、后续规划）
...
当前仅准备M4 §56的唯一**Proposed**对照：UrbanEV h3/F4/fold6 AMD-Concat、batch128/workers0及原数据/精度/seed/LR不变，S1T4、S2T4、S4T4、S4T2完成同一组task0…task3，3次预定顺序重复；共48个有限worker，每个连续8步warm-up+512步测量及4批validation，24,960个Adam步骤、192批validation。新增步数、4路、4→2计算线程、atol=1e-6/rtol=1e-5与60min/每worker10min等上限尚待用户精确批准；本轮无模型负载。复用成功工具及权限/进程退出修复；分别量同四任务端到端T_e2e与统一放行、波内结束等待的T_train，不将S4T2对S1T4解释为纯并发效应。完整数学/执行配置、资源上限、guard、外置包与无负载自检只保留于唯一M4 §56，不在此复制。不得开启正式训练、改变batch/workers/AMP/TF32/compile/MPS，或按validation改善选择调度。
