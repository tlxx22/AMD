...
2026-09-13 用户治理澄清：小规模文档更新不单独安排closure，可与下一实质工作统一提交；这不免除审核、训练来源/版本核验或启动器要求的clean Git。§§58–60累计文档已随THLS代码/测试统一closure；§62结果登记与§63接入代码/测试/策略绑定及累计文档统一待审，不单独安排docs-only commit。“两个近期论文来源模块”属于用户选择的创新组织策略，不是学校/导师硬性要求；真实来源与冻结历史不倒改，现有模型及阶段任务表不因此重排。
...
2026-09-15当前更新：THLS独立N160步检查及随后用户明确授权的UrbanEV A四H→N四H两波8-run均已完成并获接受；实际墙钟6小时16分14秒，不与旧情景估算相除冒称实测加速比。用户确认已验证并发在相同适用范围自动沿用，不再另索并发许可；UrbanEV T12证据不覆盖ETTm1 T512。M4 §62.8的A/N各四H串行/四路兼容包（320 Adam步骤）现已随新任务合同获批；§63新接入增量工程已通过，兼容包待implementation review/统一closure后执行，通过正确性/资源和同组makespan检查后直接用于该合同训练；不开展1/2/4/6/8路搜索，不将本规则写成全局性能冻结。下文旧有限对照与长期分层规划保留其适用范围。
