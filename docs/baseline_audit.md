# AMD paper-close baseline：M0-A 审计

> 审计日期：2026-08-15（UTC）
> 审计范围：仅 M0-A
> 结论状态：M0-A 已闭合；M0-B、M1 均未开始
...
3. docs/AMD_EV_Thesis_Final_Implementation_Plan_v2.1.md 已成为唯一权威方案；旧稿保留在 docs/archive/ 并明确失效，不参与实现决策。
...
| canonical v2.1 | de89194462d88a8f9d837bf8e48ee04f27142e263cbf701660c7c6bd5da118b7 |
...
两份旧方案继续保留在 docs/archive/，其归档/失效声明没有删除；只把其中的唯一权威路径修正为 docs/AMD_EV_Thesis_Final_Implementation_Plan_v2.1.md。
...
- 当前 AMD 还没有 PMCR、TEB、return_state 或 AMDEnhanced；本轮没有添加这些实现。
- 未来 M0-B 若在 TEB 关闭且 return_state=True，exo_context 必须初始化为固定零向量，以保持定义完整和 H_time 输入维度固定；该要求本轮只记录，未实现。
...
~~~text
docs/audit/upstream-main-000d377_to_fa96656.full.patch
...
## 5. ETTm1 smoke-reproduction artifact 审计
...
~~~text
docs/audit/m0a_workspace_inventory.md
...
本审计文档对应的 M0-A 已闭合。闭合时尚未创建 AMDEnhanced，尚未实现 return_state_source、PMCR 或 TEB，未修改训练行为、未启动训练，也未进入 M1。M0-B 必须在 M0-A 文档提交推送且 worktree 干净后另行执行。
