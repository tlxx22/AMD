...
class SelectivePatchTEBParallelTests(unittest.TestCase):
    @staticmethod
    ...
    @staticmethod
    def _formal_components(module, hidden, normalized):
        ...
        context_flat, _ = module.cross_attention(
            query,
            exogenous,
            exogenous,
        ...
        patch_attention = context[:, :, : module.num_patches]
        global_attention = context[:, :, module.num_patches]
        raw, gate, effective = module.compute_patch_delta_components(
            patch_query, patch_attention
        )
        return patch_query, patch_attention, global_attention, raw, gate, effective

    def test_parallel_shapes_mask_and_initial_gate(self):
        ...
        query, attention, _, raw, gate, effective = self._formal_components(
            module, hidden, normalized
        )
        self.assertEqual(query.shape, (2, 3, 2, 8))
        self.assertEqual(attention.shape, (2, 3, 2, 8))
        self.assertEqual(raw.shape, (2, 3, 2, 3))
        self.assertEqual(gate.shape, (2, 3, 2, 1))
        self.assertEqual(effective.shape, (2, 3, 2, 3))
        ...
        owner = torch.arange(3).repeat_interleave(queries_per_variable)
        expected_mask = torch.arange(3).unsqueeze(0).eq(owner.unsqueeze(1))
        mask = module._parallel_attention_mask(torch.device("cpu"))
        self.assertTrue(torch.equal(mask, expected_mask))
        self.assertEqual(mask.shape, (3 * queries_per_variable, 3))
    ...
    def test_one_vectorized_attention_call_and_every_variable_updates(self):
    ...
    def test_target_idx_only_selects_global_context(self):
        ...
        with torch.no_grad():
            ...
            components = self._formal_components(first, hidden, normalized)
            global_all = components[2]
    ...
    def test_variable_permutation_equivariance_and_shared_gate_parameters(self):
        torch.manual_seed(6233)
        ...
        self.assertEqual(
            tuple(original.patch_confidence_gate_weight.shape), (1, 16)
        )
        self.assertEqual(
            tuple(wider.patch_confidence_gate_weight.shape), (1, 16)
        )
        self.assertEqual(
    ...
    def test_owner_token_is_masked_but_changes_other_variables(self):
        torch.manual_seed(6247)
        module = self._module().eval()
        hidden = torch.randn(2, 3, 5)
        normalized = torch.randn(2, 5, 3)
        modified = normalized.clone()
        modified[:, :, 1] += torch.tensor([0.0, 3.0, -2.0, 5.0, -4.0])
        with torch.no_grad():
            before, _ = module(hidden, normalized)
            after, _ = module(hidden, modified)
        self.assertTrue(torch.equal(before[:, 1], after[:, 1]))
        self.assertGreater(torch.stack((
            (before[:, 0] - after[:, 0]).abs().max(),
            (before[:, 2] - after[:, 2]).abs().max(),
        )).max().item(), 0.0)

    def test_parallel_c1_and_nonempty_aux_are_rejected(self):
